                                          ...
                                          Suin_FTPClient_ObservableInterface
{
	...
	/**
	 * Return a list of files in the given directory and all subdirectories.
	 * @param string $directory
	 * @return array|bool If error, returns FALSE.
	 */
    public function getRecursiveList($directory)
    {
		$dataConnection = $this->_openPassiveDataConnection();
		...
		$response = $this->_request(sprintf('LIST -R %s', $directory));
        ...
        {
            if (preg_match('_.*:[0-9][0-9] (.*)_',$line,$match))
            {
                $file=$match[1];
